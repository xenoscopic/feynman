...
def test_integral_generation():
    ...
    for integrand in input_code.function_declarations:
        ...
        assert(integral.n_dimensions == len(integrand.argument_types))
        arg_i = 0
...
def test_file_name_parsing():
    ...
    #Grab the first function declaration
    integrand = input_code.function_declarations[0]
    ...
    name_parsed_pairs = (
        ("Integrand", "Integrand"),
        ("Integrand22", "Integrand22"),
        ("Integrand Five", "IntegrandFive"),
        ("Integrand$Six", "IntegrandSix"),
        (" Integrand *(*\t", "Integrand")
    )

    for name, parsed in name_parsed_pairs:
        #Create the integrator
        integrator = integration.FunctionIntegrator(integral, name)

        #Make sure it did the correct thing with the name
        assert(integrator.file_name == parsed)
